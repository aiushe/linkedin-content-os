...
| Retrieval | Ranked story candidates by verified-metric overlap before general lexical relevance. | C2 | Story top-1 +15pp, with a risk to topical-but-nonmetric cases. | **Regression:** top-1 **52.38% → 47.62%** (-4.76pp) and top-3 **76.19% → 71.43%** (-4.76pp). Keep the result as evidence that metric overlap is too narrow a first-rank signal; do not retain this ordering without a hybrid ranker. |
